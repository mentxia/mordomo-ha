...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    internal_webhook_url = f"http://127.0.0.1:8123/api/webhook/{webhook_id}"
    ...
    # Start Baileys bridge if using direct connection
    if wa_gateway_type == "baileys_direct" and hasattr(wa, "start_bridge"):
        _LOGGER.info("Starting Baileys WhatsApp bridge...")
        bridge_ok = await wa.start_bridge()
        if not bridge_ok:
            _LOGGER.warning(
                "Baileys bridge failed to start. "
                "Ensure Node.js >= 18 is installed. "
                "WhatsApp will not work until bridge is running."
            )
